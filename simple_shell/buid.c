...
int custom_cd(ShellData *shell_info)
{
    ...
    {
        val = chdir(_getenv("OLDPWD=", shell_info) ? _getenv("OLDPWD=", shell_info) : oldwd);
    ...
    if (1)
    {
        shell_info->argument_array[1] = "OLDPWD", shell_info->argument_array[2] = oldwd;
        _getenv("OLDPWD", shell_info);
        shell_info->argument_array[1] = "PWD", shell_info->argument_array[2] = cwd = getcwd(cwd, 0);
        _setenv(shell_info);
        free(cwd);
    }
    free(oldwd);

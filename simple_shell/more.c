...
void error(ShellData *shell_info, int errortype)
{
    ...
    printerr(shell_info->argv), printerr(": ");
    if (errno == ENOTDIR || errno == ENOENT || errno == ENOTTY)
    {
        printerr(number), printerr(": "), printerr(shell_info->argument_array[0]);
    ...
    {
        printerr(number), printerr(": "), printerr(shell_info->argument_array[0]);
        printerr(": Illegal number: "), printerr(shell_info->argument_array[1]), printerr("\n");
    }
    else
    {
        printerr(number), printerr(": "),
            printerr(shell_info->argument_array[0]), printerr(": ");
        perror(NULL);
    }
}
...
void _shell(ShellData *shell_info)
{
    ...
    while (1)
    {
        ...
        if (get == -1)
        {
            if (isatty(STDIN_FILENO))
                puts("\n");
            free(shell_info->command_buffer), free(shell_info->command_history), exit(shell_info->exit_status);
        }
        pt1 = shell_info->command_buffer, pt2 = strchr(pt1, ';');
        if (pt2)
            *pt2++ = '\0';
        while (pt1)
        {
            shell_info->command_count++, shell_info->argument_array = tokarr(comments(pt1));
    ...
    free(shell_info->command_buffer), free(shell_info->command_history), free(shell_info->argument_array);
}
